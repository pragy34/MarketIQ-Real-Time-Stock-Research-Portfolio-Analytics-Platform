...
async function renderPeers() {
  ...
  if (!sector) {
    chart.innerHTML = `<p class="empty-note">No sector selected yet.</p>`;
    return;
  }

  let averages, stocks;
  try {
    ({ averages, stocks } = await Api.sector(sector));
  } catch (e) {
    chart.innerHTML = `<p class="empty-note">Couldn't load peer data: ${escapeHtml(e.message)}</p>`;
    return;
  }
