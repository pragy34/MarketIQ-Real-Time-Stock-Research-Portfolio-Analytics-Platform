...
async function loadSectorOptions() {
  let sectors = [];
  try {
    sectors = Object.keys(await Api.sectors());
  } catch (e) {
    console.warn("Could not load sector list:", e.message);
    return sectors;
  }
...
async function renderScreener() {
  ...
  let count, stocks;
  try {
    ({ count, stocks } = await Api.screener(params));
  } catch (e) {
    tbody.innerHTML = "";
    document.getElementById("screener-count").textContent = `Couldn't load screener results: ${e.message}`;
    return;
  }
