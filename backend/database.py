...
def get_holdings(session_id: str):
    # NOTE: deliberately not `SELECT *` — session_id is also the value used
    # for the httponly ml_session cookie, so it must never be echoed back
    # into a JSON response body (that would defeat httponly's whole purpose:
    # any script, including an XSS payload, could read it from the response
    # and replay it as a forged cookie).
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT id, ticker, quantity, buy_price, buy_date FROM portfolio WHERE session_id = ?",
            (session_id,),
        ).fetchall()
        return [dict(r) for r in rows]
