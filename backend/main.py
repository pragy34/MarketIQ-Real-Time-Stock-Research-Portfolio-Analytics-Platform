...
Then open http://localhost:8000 — this app also serves the frontend as
static files, so the API and UI are same-origin in local dev.
"""
import asyncio
import logging
import re
...
def market_status():
    """NSE cash-session window (approximate, no holiday calendar). Purely
    informational — a 'Market open/closed' chip in the header. Never gates
    data availability, since cached fundamentals are always served."""
    now_ist = datetime.now(IST)
    open_t = now_ist.replace(
        hour=config.MARKET_OPEN_HOUR_IST, minute=config.MARKET_OPEN_MINUTE_IST, second=0, microsecond=0
    )
    close_t = now_ist.replace(
        hour=config.MARKET_CLOSE_HOUR_IST, minute=config.MARKET_CLOSE_MINUTE_IST, second=0, microsecond=0
    )
    ...
    return {
        ...
        "next_open_hint": "09:15 IST, Mon–Fri" if not is_open else None,
    }
...
def add_watchlist(item: WatchlistAdd, request: Request):
    existing = database.get_watchlist(_session(request))
    if len(existing) >= 100:
        raise HTTPException(400, "Watchlist limit reached (100 tickers)")
...
# ---------------- Portfolio ----------------
@app.get("/api/portfolio/template", response_class=PlainTextResponse)
def portfolio_template():
    """A blank CSV with the right headers, for the upload form's 'download
    template' link."""
    return (
...
def clear_portfolio(request: Request):
    database.clear_portfolio(_session(request))
    return {"cleared": True}
